...
local files_KENAEM = database:smembers("files"..bot_id) 
print(Green.."\nFiles Now Started : \n "..reset)
for i,v in pairs(files_KENAEM) do
print(Blue..i..red..' - \27[10;33m'..v..',\27[m')  end
print(Green.."\nThes all Files.\n\n\n"..reset)
io.popen("mkdir files_KENAEM")
...
local keko_add_sudo = redis:get('KENAEM:'..bot_id..'sudoo'..msg.sender_user_id_..'')
...
local var = false 
local admin = database:sismember('KENAEM:'..bot_id..'admins:', user_id)
if admin then var = true end
for k,v in pairs(sudo_users) do
if user_id == v then var = true end
end
local keko_add_sudo = redis:get('KENAEM:'..bot_id..'sudoo'..user_id..'')
...
local var = false
local creator = database:sismember('KENAEM:'..bot_id..'creator:'..chat_id, user_id) 
local admin = database:sismember('KENAEM:'..bot_id..'admins:', user_id)
...
chat_id = msg.chat_id_
local var = false
local mod = database:sismember('KENAEM:'..bot_id..'mods:'..chat_id, user_id)  
local admin = database:sismember('KENAEM:'..bot_id..'admins:', user_id)  
local owner = database:sismember('KENAEM:'..bot_id..'owners:'..chat_id, user_id)
local creator = database:sismember('KENAEM:'..bot_id..'creator:'..chat_id, user_id)  
local vip = database:sismember('KENAEM:'..bot_id..'vipgp:'..chat_id, user_id)
...
local var = false
local banned = database:sismember('KENAEM:'..bot_id..'banned:'..chat_id, user_id)
...
local var = false
local banned = database:sismember('KENAEM:'..bot_id..'gbanned:', user_id)
if banned then var = true end
return var
end
--         »»                 is_muted                         ««              --
...
local var = false
local banned = database:sismember('KENAEM:'..bot_id..'muted:'..chat_id, user_id)
if banned then var = true end
return var
end
--         »»                 is_gmuted                         ««              --
...
local var = false 
local banned = database:sismember('KENAEM:'..bot_id..'gmuted:', user_id)
if banned then var = true end
return var
end
--         »»                 getMessage                         ««              --
...
local hash =  'KENAEM:'..bot_id..'filters:'..msg.chat_id_
if hash then
...
local function send(chat_id, reply_to_message_id, disable_notification, text, disable_web_page_preview, parse_mode)
  ...
  {
  {text = ''..(channel_ts or "KENAEM TEAM")..'', url=''..(channel_user_ts or 't.me/KENAEMTEAM')..''},
  },
  }
...
info = '💬┇بواسطه ~⪼ ['..result.first_name_..'](t.me/'..(result.username_ or 'KENAEMteam')..')\n'..text
send(msg.chat_id_, msg.id_, 1,info, 1, 'md')
 end
getUser(msg.sender_user_id_, keko333)
end
if value == "prore" then
function get_KENAEMX(KENAEMx1,KENAEMx2,KENAEMx3)
local id_KENAEMx = KENAEMx2.sender_user_id_
function keko333(extra,result,success)
info = '👤┇العضو ~⪼ ['..result.first_name_..'](t.me/'..(result.username_ or 'KENAEMteam')..')\n'..text
send(msg.chat_id_, msg.id_, 1,info, 1, 'md')
 end
getUser(id_KENAEMx, keko333)
end
getMessage(msg.chat_id_, msg.reply_to_message_id_,get_KENAEMX)
end
if value ~= "prore" and value~= "lock"  then
function keko333(extra,result,success)
info = '👤┇العضو ~⪼ ['..(result.first_name_ or value)..'](t.me/'..(result.username_ or 'KENAEMteam')..')\n'..text
send(msg.chat_id_, msg.id_, 1,info, 1, 'md')
end
...
if not database:get( 'KENAEM:'..bot_id.."charge:"..msg.chat_id_) then
function KENAEM_info(k1,k2)
send(msg.chat_id_, msg.id_, 1, "❕┇المجموعه {"..(k2.title_ or "").."} معطله سابقا", 1, 'md')
end
openChat(msg.chat_id_,KENAEM_info)
end
if database:get( 'KENAEM:'..bot_id.."charge:"..msg.chat_id_) then
database:del( 'KENAEM:'..bot_id.."charge:"..msg.chat_id_)
function KENAEM_info(k1,k2)
send(msg.chat_id_, msg.id_, 1, info.."☑️┇تم تعطيل المجموعه {"..k2.title_.."}", 1, 'md')
end
openChat(msg.chat_id_,KENAEM_info)
database:srem("KENAEM:gog"..bot_id, msg.chat_id_)
database:del( 'KENAEM:'..bot_id.."charge:"..msg.chat_id_)
function KENAEM_info2(k1,k2)
local getlink = 'https://api.telegram.org/bot'..token..'/exportChatInviteLink?chat_id='..msg.chat_id_
local req = https.request(getlink)
local link = json:decode(req)
if link.ok == true then 
database:set('KENAEM:'..bot_id.."group:link"..msg.chat_id_ ,link.result) 
link = link.result 
else 
link = "(لا يوجد)"
end
send(tostring((database:get("KENAEM"..bot_id..":sudo:gr") or sudo_add)), 0, 1, "🔘┇قام بتعطيل مجموعه \n🎫┇ايدي المطور ~⪼ ("..msg.sender_user_id_..")\n📜┇معرف المطور ~⪼ @"..(result.username_ or "لا يوجد").."\n🌐┇معلومات المجموعه \n\n🎫┇ايدي المجموعه ~⪼ ("..msg.chat_id_..")\nⓂ️┇اسم المجموعه ~⪼ ("..k2.title_..")\n📎┇رابط المجموعه ~⪼ ["..link.."]" , 1, 'html')
end
openChat(msg.chat_id_,KENAEM_info2) 
--
end
else
...
if not database:get( 'KENAEM:'..bot_id.."charge:"..msg.chat_id_) then
database:set( 'KENAEM:'..bot_id.."charge:"..msg.chat_id_,true)
function KENAEM_info(k1,k2)
send(msg.chat_id_, msg.id_, 1, info.."☑️┇تم تفعيل المجموعه {"..(k2.title_ or "").."}", 1, 'md')
end
openChat(msg.chat_id_,KENAEM_info)
database:sadd("KENAEM:gog"..bot_id, msg.chat_id_)
function KENAEM_info2(k1,k2)
local getlink = 'https://api.telegram.org/bot'..token..'/exportChatInviteLink?chat_id='..msg.chat_id_
local req = https.request(getlink)
local link = json:decode(req)
...
send(tostring((database:get("KENAEM"..bot_id..":sudo:gr") or sudo_add)), 0, 1, "🔘┇قام بتفعيل مجموعه \n🎫┇ايدي المطور ~⪼ ("..msg.sender_user_id_..")\n📜┇معرف المطور ~⪼ @"..(result.username_ or "لا يوجد").."\n🌐┇معلومات المجموعه \n\n🎫┇ايدي المجموعه ~⪼ ("..msg.chat_id_..")\nⓂ️┇اسم المجموعه ~⪼ ("..k2.title_..")\n📎┇رابط المجموعه ~⪼ ["..link.."]" , 1, 'html')
end
openChat(msg.chat_id_,KENAEM_info2) 
--
if data.result.can_promote_members  then
database:sadd('KENAEM:'..bot_id..'owners:'..msg.chat_id_,msg.sender_user_id_)
end
database:set( 'KENAEM:'..bot_id.."enable:"..msg.chat_id_,true)
if data.result.status == 'creator' then
database:sadd('KENAEM:'..bot_id..'creator:'..msg.chat_id_, msg.sender_user_id_)
end
end
else
...
if (text == "قفل التاك") then
  local tsX_o = database:get("lock_tag:KENAEM"..msg.chat_id_..bot_id)
  if tsX_o then
  tsX000("lock",msg,"☑┇بالفعل تم قفل التاك")
  else
  tsX000("lock",msg,"☑┇تم قفل التاك")
  database:set("lock_tag:KENAEM"..msg.chat_id_..bot_id,"ok")
  end
  end
  if (text == "قفل الشارحه") then
  local tsX_o = database:get("lock_sarha:KENAEM"..msg.chat_id_..bot_id)
  if tsX_o then
  tsX000("lock",msg,"☑┇بالفعل تم قفل الشارحه")
  else
  tsX000("lock",msg,"☑┇تم قفل الشارحه")
  database:set("lock_sarha:KENAEM"..msg.chat_id_..bot_id,"ok")
  end
  end
  if (text == "قفل التعديل") then
  local tsX_o = database:get("lock_edit:KENAEM"..msg.chat_id_..bot_id)
  if tsX_o then
  tsX000("lock",msg,"☑┇بالفعل تم قفل التعديل")
  else
  tsX000("lock",msg,"☑┇تم قفل التعديل")
  database:set("lock_edit:KENAEM"..msg.chat_id_..bot_id,"ok")
  end
  end
  if (text == "قفل التكرار") then
  local tsX_o = database:get("lock_lllll:KENAEM"..msg.chat_id_..bot_id)
  if tsX_o then
  tsX000("lock",msg,"☑┇بالفعل تم قفل التكرار")
  else
  tsX000("lock",msg,"☑┇تم قفل التكرار")
  database:set("lock_lllll:KENAEM"..msg.chat_id_..bot_id,"ok")
  end
  end
  if (text == "قفل المتحركه") then
  local tsX_o = database:get("lock_gif:KENAEM"..msg.chat_id_..bot_id)
  if tsX_o then
  tsX000("lock",msg,"☑┇بالفعل تم قفل المتحركه")
  else
  tsX000("lock",msg,"☑┇تم قفل المتحركه")
  database:set("lock_gif:KENAEM"..msg.chat_id_..bot_id,"ok")
  end
  end
  if (text == "قفل الملفات") then
  local tsX_o = database:get("lock_files:KENAEM"..msg.chat_id_..bot_id)
  if tsX_o then
  tsX000("lock",msg,"☑┇بالفعل تم قفل الملفات")
  else
  tsX000("lock",msg,"☑┇تم قفل الملفات")
  database:set("lock_files:KENAEM"..msg.chat_id_..bot_id,"ok")
  end
  end
  if (text == "قفل الماركدون") then
  local tsX_o = database:get("lock_mark:KENAEM"..msg.chat_id_..bot_id)
  if tsX_o then
  tsX000("lock",msg,"☑┇بالفعل تم قفل الماركدون")
  else
  tsX000("lock",msg,"☑┇تم قفل الماركدون")
  database:set("lock_mark:KENAEM"..msg.chat_id_..bot_id,"ok")
  end
  end
  if (text == "قفل الصور") then
  local tsX_o = database:get("lock_photo:KENAEM"..msg.chat_id_..bot_id)
  if tsX_o then
  tsX000("lock",msg,"☑┇بالفعل تم قفل الصور")
  else
  tsX000("lock",msg,"☑┇تم قفل الصور")
  database:set("lock_photo:KENAEM"..msg.chat_id_..bot_id,"ok")
  end
  end
  if (text == "قفل الملصقات") then
  local tsX_o = database:get("lock_stecker:KENAEM"..msg.chat_id_..bot_id)
  if tsX_o then
  tsX000("lock",msg,"☑┇بالفعل تم قفل الملصقات")
  else
  tsX000("lock",msg,"☑┇تم قفل الملصقات")
  database:set("lock_stecker:KENAEM"..msg.chat_id_..bot_id,"ok")
  end
  end
  if (text == "قفل الفيديو") then
  local tsX_o = database:get("lock_video:KENAEM"..msg.chat_id_..bot_id)
  if tsX_o then
  tsX000("lock",msg,"☑┇بالفعل تم قفل الفيديو")
  else
  tsX000("lock",msg,"☑┇تم قفل الفيديو")
  database:set("lock_video:KENAEM"..msg.chat_id_..bot_id,"ok")
  end
  end
  if (text == "قفل الانلاين") then
  local tsX_o = database:get("lock_inline:KENAEM"..msg.chat_id_..bot_id)
  if tsX_o then
  tsX000("lock",msg,"☑┇بالفعل تم قفل الانلاين")
  else
  tsX000("lock",msg,"☑┇تم قفل الانلاين")
  database:set("lock_inline:KENAEM"..msg.chat_id_..bot_id,"ok")
  end
  end
  if (text == "قفل الدردشه") then
  local tsX_o = database:get("lock_chat:KENAEM"..msg.chat_id_..bot_id)
  if tsX_o then
  tsX000("lock",msg,"☑┇بالفعل تم قفل الدردشه")
  else
  tsX000("lock",msg,"☑┇تم قفل الدردشه")
  database:set("lock_chat:KENAEM"..msg.chat_id_..bot_id,"ok")
  end
  end
  if (text == "قفل التوجيه") then
  local tsX_o = database:get("lock_fwd:KENAEM"..msg.chat_id_..bot_id)
  if tsX_o then
  tsX000("lock",msg,"☑┇بالفعل تم قفل التوجيه")
  else
  tsX000("lock",msg,"☑┇تم قفل التوجيه")
  database:set("lock_fwd:KENAEM"..msg.chat_id_..bot_id,"ok")
  end
  end
  if (text == "قفل التثبيت") then
  local tsX_o = database:get("lock_pin:KENAEM"..msg.chat_id_..bot_id)
  if tsX_o then
  tsX000("lock",msg,"☑┇بالفعل تم قفل التثبيت")
  else
  tsX000("lock",msg,"☑┇تم قفل التثبيت")
  database:set("lock_pin:KENAEM"..msg.chat_id_..bot_id,"ok")
  end
  end
  if (text == "قفل الاغاني") then
  local tsX_o = database:get("lock_audeo:KENAEM"..msg.chat_id_..bot_id)
  if tsX_o then
  tsX000("lock",msg,"☑┇بالفعل تم قفل الاغاني")
  else
  tsX000("lock",msg,"☑┇تم قفل الاغاني")
  database:set("lock_audeo:KENAEM"..msg.chat_id_..bot_id,"ok")
  end
  end
  if (text == "قفل الصوت") then
  local tsX_o = database:get("lock_voice:KENAEM"..msg.chat_id_..bot_id)
  if tsX_o then
  tsX000("lock",msg,"☑┇بالفعل تم قفل الصوت")
  else
  tsX000("lock",msg,"☑┇تم قفل الصوت")
  database:set("lock_voice:KENAEM"..msg.chat_id_..bot_id,"ok")
  end
  end
  if (text == "قفل الجهات") then
  local tsX_o = database:get("lock_contact:KENAEM"..msg.chat_id_..bot_id)
  if tsX_o then
  tsX000("lock",msg,"☑┇بالفعل تم قفل الجهات")
  else
  tsX000("lock",msg,"☑┇تم قفل الجهات")
  database:set("lock_contact:KENAEM"..msg.chat_id_..bot_id,"ok")
  end
  end
  if (text == "قفل العربيه") then
  local tsX_o = database:get("lock_ar:KENAEM"..msg.chat_id_..bot_id)
  if tsX_o then
    tsX000("lock",msg,"☑┇بالفعل تم قفل العربيه")
  else
  tsX000("lock",msg,"☑┇تم قفل العربيه")
  database:set("lock_ar:KENAEM"..msg.chat_id_..bot_id,"ok")
  end
  end
  if (text == "قفل الانكليزيه") then
  local tsX_o = database:get("lock_en:KENAEM"..msg.chat_id_..bot_id)
  if tsX_o then
  tsX000("lock",msg,"☑┇بالفعل تم قفل الانكليزيه")
  else
  tsX000("lock",msg,"☑┇تم قفل الانكليزيه")
  database:set("lock_en:KENAEM"..msg.chat_id_..bot_id,"ok")
  end
  end
  if (text == "قفل الميديا") then
  local tsX_o = database:get("lock_media:KENAEM"..msg.chat_id_..bot_id)
  if tsX_o then
  tsX000("lock",msg,"☑┇بالفعل تم قفل الميديا")
  else
  tsX000("lock",msg,"☑┇تم قفل الميديا")
  database:set("lock_media:KENAEM"..msg.chat_id_..bot_id,"ok")
database:set("lock_audeo:KENAEM"..msg.chat_id_..bot_id,"ok")
database:set("lock_video:KENAEM"..msg.chat_id_..bot_id,"ok")
database:set("lock_photo:KENAEM"..msg.chat_id_..bot_id,"ok")
database:set("lock_stecker:KENAEM"..msg.chat_id_..bot_id,"ok")
database:set("lock_voice:KENAEM"..msg.chat_id_..bot_id,"ok")
database:set("lock_gif:KENAEM"..msg.chat_id_..bot_id,"ok")
database:set("lock_note:KENAEM"..msg.chat_id_..bot_id,"ok")
  end
  end
  if (text == "قفل الروابط") then
  local tsX_o = database:get("lock_link:KENAEM"..msg.chat_id_..bot_id)
  if tsX_o then
    tsX000("lock",msg,"☑┇بالفعل تم قفل الروابط")
  else
  tsX000("lock",msg,"☑┇تم قفل الروابط")
  database:set("lock_link:KENAEM"..msg.chat_id_..bot_id,"ok")
  end
  end
  if (text == "قفل المعرف") then
  local tsX_o = database:get("lock_username:KENAEM"..msg.chat_id_..bot_id)
  if tsX_o then
   tsX000("lock",msg,"☑┇بالفعل تم قفل المعرف")
  else
  tsX000("lock",msg,"☑┇تم قفل المعرف")
  database:set("lock_username:KENAEM"..msg.chat_id_..bot_id,"ok")
  end
  end
  if (text == "قفل الاشعارات") then
  local tsX_o = database:get("lock_new:KENAEM"..msg.chat_id_..bot_id)
  if tsX_o then
  tsX000("lock",msg,"☑┇بالفعل تم قفل الاشعارات")
  else
  tsX000("lock",msg,"☑┇تم قفل الاشعارات")
  database:set("lock_new:KENAEM"..msg.chat_id_..bot_id,"ok")
  end
  end
  if (text == "قفل البوتات بالطرد") then
  local tsX_o = database:get("lock_botAndBan:KENAEM"..msg.chat_id_..bot_id)
  if tsX_o then
   tsX000("lock",msg,"☑┇بالفعل تم قفل البوتات بالطرد")
  else
  tsX000("lock",msg,"☑┇تم قفل البوتات بالطرد")
  database:set("lock_botAndBan:KENAEM"..msg.chat_id_..bot_id,"ok")
  end
  end
  if (text == "قفل البوتات") then
  local tsX_o = database:get("lock_bot:KENAEM"..msg.chat_id_..bot_id)
  if tsX_o then
  tsX000("lock",msg,"☑┇بالفعل تم قفل البوتات")
  else
  tsX000("lock",msg,"☑┇تم قفل البوتات")
  database:set("lock_bot:KENAEM"..msg.chat_id_..bot_id,"ok")
  end
  end
  if (text == "قفل بصمه الفيديو") then
  local tsX_o = database:get("lock_note:KENAEM"..msg.chat_id_..bot_id)
  if tsX_o then
  ...
  database:del("lock_new:KENAEM"..msg.chat_id_..bot_id,"ok")
  end
  end
  ...
  database:del("lock_tag:KENAEM"..msg.chat_id_..bot_id,"ok")
  end
  end
  ...
  database:del("lock_sarha:KENAEM"..msg.chat_id_..bot_id,"ok")
  end
  end
  ...
  database:del("lock_lllll:KENAEM"..msg.chat_id_..bot_id,"ok")
  end
  end
  ...
  database:del("lock_edit:KENAEM"..msg.chat_id_..bot_id,"ok")
  end
  end
  ...
  database:del("lock_gif:KENAEM"..msg.chat_id_..bot_id,"ok")
  end
  end
  ...
  database:del("lock_files:KENAEM"..msg.chat_id_..bot_id,"ok")
  end
  end
  ...
  database:del("lock_mark:KENAEM"..msg.chat_id_..bot_id,"ok")
  end
  end
  ...
  database:del("lock_photo:KENAEM"..msg.chat_id_..bot_id,"ok")
  end
  end
  ...
  database:del("lock_stecker:KENAEM"..msg.chat_id_..bot_id,"ok")
  end
  end
  ...
  database:del("lock_video:KENAEM"..msg.chat_id_..bot_id,"ok")
  end
  end
  ...
  database:del("lock_inline:KENAEM"..msg.chat_id_..bot_id,"ok")
  end
  end
  ...
  database:del("lock_chat:KENAEM"..msg.chat_id_..bot_id,"ok")
  end
  end
  ...
  database:del("lock_fwd:KENAEM"..msg.chat_id_..bot_id,"ok")
  end
  end
  ...
  database:del("lock_pin:KENAEM"..msg.chat_id_..bot_id,"ok")
  end
  end
  ...
  database:del("lock_audeo:KENAEM"..msg.chat_id_..bot_id,"ok")
  end
  end
  ...
  database:del("lock_voice:KENAEM"..msg.chat_id_..bot_id,"ok")
  end
  end
  ...
  database:del("lock_botAndBan:KENAEM"..msg.chat_id_..bot_id,"ok")
  end
  end
  ...
  database:del("lock_contact:KENAEM"..msg.chat_id_..bot_id,"ok")
  end
  end
  ...
  database:del("lock_ar:KENAEM"..msg.chat_id_..bot_id,"ok")
  end
  end
  ...
  database:del("lock_en:KENAEM"..msg.chat_id_..bot_id,"ok")
  end
  end
  ...
  database:del("lock_media:KENAEM"..msg.chat_id_..bot_id,"ok")
database:del("lock_audeo:KENAEM"..msg.chat_id_..bot_id,"ok")
database:del("lock_video:KENAEM"..msg.chat_id_..bot_id,"ok")
database:del("lock_photo:KENAEM"..msg.chat_id_..bot_id,"ok")
database:del("lock_stecker:KENAEM"..msg.chat_id_..bot_id,"ok")
database:del("lock_voice:KENAEM"..msg.chat_id_..bot_id,"ok")
database:del("lock_gif:KENAEM"..msg.chat_id_..bot_id,"ok")
database:del("lock_note:KENAEM"..msg.chat_id_..bot_id,"ok")
  end
  end
  ...
  database:del("lock_username:KENAEM"..msg.chat_id_..bot_id,"ok")
  end
  end
  ...
  database:del("lock_link:KENAEM"..msg.chat_id_..bot_id,"ok")
  end
  end
  ...
  database:del("lock_bot:KENAEM"..msg.chat_id_..bot_id,"ok")
  end
  end  --         »»               End KENAEM unlock                       ««              --
  end 
end 
...
if database:get("lock_link:KENAEM"..msg.chat_id_..bot_id) then
...
if database:get("lock_username:KENAEM"..msg.chat_id_..bot_id) then
delete_msg(msg.chat_id_,{[0] = msg.id_})
end
end
end
...
if database:get("lock_chat:KENAEM"..msg.chat_id_..bot_id) then
delete_msg(msg.chat_id_,{[0] = msg.id_})
return "KENAEM"
end
if text and text:match("(.*)(/)(.*)")  then
if database:get("lock_sarha:KENAEM"..msg.chat_id_..bot_id) then
delete_msg(msg.chat_id_,{[0] = msg.id_})
end
end
if text and text:match("(.*)(#)(.*)")  then
if database:get("lock_tag:KENAEM"..msg.chat_id_..bot_id) then
delete_msg(msg.chat_id_,{[0] = msg.id_})
end
end
if msg.content_.ID == "MessageChatAddMembers" then
if msg.content_.members_[0].type_.ID == 'UserTypeBot' then
if database:get("lock_bot:KENAEM"..msg.chat_id_..bot_id) then
changeChatMemberStatus(msg.chat_id_, msg.content_.members_[0].id_, "Kicked")
end
if database:get("lock_botAndBan:KENAEM"..msg.chat_id_..bot_id) then
changeChatMemberStatus(msg.chat_id_, msg.content_.members_[0].id_, "Kicked")
...
if database:get("lock_fwd:KENAEM"..msg.chat_id_..bot_id) then
delete_msg(msg.chat_id_,{[0] = msg.id_})
end
end
end
if msg.content_.ID == "MessageSticker" then
if database:get("lock_stecker:KENAEM"..msg.chat_id_..bot_id) then
delete_msg(msg.chat_id_,{[0] = msg.id_})
end
end
if msg.content_.ID == "MessageChatJoinByLink" or msg.content_.ID == "MessageChatAddMembers" then
if database:get("lock_new:KENAEM"..msg.chat_id_..bot_id) then
delete_msg(msg.chat_id_,{[0] = msg.id_})
end
end

if msg.content_.ID == "MessageChatAddMembers" then
database:incr('KENAEM:'..bot_id..'user:add'..msg.chat_id_..':'..msg.sender_user_id_)
end
if msg.content_.ID == "MessageUnsupported" then
if database:get("lock_note:KENAEM"..msg.chat_id_..bot_id) then
delete_msg(msg.chat_id_,{[0] = msg.id_})
end
end
if msg.content_.ID == "MessagePhoto" then
if database:get("lock_photo:KENAEM"..msg.chat_id_..bot_id) then
delete_msg(msg.chat_id_,{[0] = msg.id_})
end
end
if msg.content_.ID == "MessageAudio" then
if database:get("lock_audeo:KENAEM"..msg.chat_id_..bot_id) then
delete_msg(msg.chat_id_,{[0] = msg.id_})
end
end
if msg.content_.ID == "MessageVoice" then
if database:get("lock_voice:KENAEM"..msg.chat_id_..bot_id) then
delete_msg(msg.chat_id_,{[0] = msg.id_})
end
end
if msg.content_.ID == "MessageVideo" then
if database:get("lock_video:KENAEM"..msg.chat_id_..bot_id) then
delete_msg(msg.chat_id_,{[0] = msg.id_})
end
end
if msg.content_.ID == "MessageAnimation" then
if database:get("lock_gif:KENAEM"..msg.chat_id_..bot_id) then
delete_msg(msg.chat_id_,{[0] = msg.id_})
end
end
if msg.content_.ID == "MessageContact" then
if database:get("lock_contact:KENAEM"..msg.chat_id_..bot_id) then
delete_msg(msg.chat_id_,{[0] = msg.id_})
end
end
if text and text:match("[\216-\219][\128-\191]") then
if database:get("lock_ar:KENAEM"..msg.chat_id_..bot_id) then
delete_msg(msg.chat_id_,{[0] = msg.id_})
end
end
if msg.content_.ID == "MessageDocument" then
if database:get("lock_files:KENAEM"..msg.chat_id_..bot_id) then
delete_msg(msg.chat_id_,{[0] = msg.id_})
end
end
if text and text:match("[ASDFGHJKLQWERTYUIOPZXCVBNMasdfghjklqwertyuiopzxcvbnm]") then
if database:get("lock_en:KENAEM"..msg.chat_id_..bot_id) then
delete_msg(msg.chat_id_,{[0] = msg.id_})
end
end
...
if database:get("lock_mark:KENAEM"..msg.chat_id_..bot_id) then
delete_msg(msg.chat_id_,{[0] = msg.id_})
end
end
end
...
if database:get("lock_lllll:KENAEM"..msg.chat_id_..bot_id) then
...
local hash = 'KENAEM:'..bot_id..'flood:time:'..msg.chat_id_
if not database:get(hash) then
floodTime = 1
else
...
database:sadd('KENAEM:'..bot_id..'muted:'..msg.chat_id_, msg.sender_user_id_)
end
end
...
if not database:get('KENAEM:'..bot_id..'leave:groups') then
...
local hash =  'KENAEM:'..bot_id..'creator:'..msg.chat_id_
...
if result.id_ then
database:sadd('KENAEM:'..bot_id..'creator:'..msg.chat_id_, result.id_)
texts = '👤┇العضو ~⪼ ['..result.title_..'](t.me/'..(apow[2] or 'KENAEMteam')..')\n☑┇تم رفع منشئ في البوت'
else
texts = '✖┇خطاء'
end
...
database:sadd('KENAEM:'..bot_id..'creator:'..msg.chat_id_, apow[2])
...
local user_info = database:hgetall('KENAEM:'..bot_id..'user:'..v)
...
if not database:get('KENAEM:'..bot_id..'bc:groups') then
local gps = database:scard( 'KENAEM:'..bot_id.."groups") or 0
local gpss = database:smembers( 'KENAEM:'..bot_id.."groups") or 0
for i=1, #gpss do
if not database:sismember('KENAEM:'..bot_id..'pro:groups', gpss[i]) then
send(gpss[i], 0, 1, ssss[2], 1, 'html')		
end					
end
local pro = database:scard('KENAEM:'..bot_id..'pro:groups') or 0
...
local gps = database:scard('KENAEM:'..bot_id.."groups") or 0
local user = database:scard('KENAEM:'..bot_id.."userss") or 0
local gps2 = database:scard("KENAEM:gog"..bot_id) or 0
local gps3 = database:scard('KENAEM:'..bot_id..'pro:groups') or 0
...
local gps = database:smembers('KENAEM:'..bot_id.."groups") or 0
local gps2 = database:smembers("KENAEM:gog"..bot_id) or 0
for i=1,#gps do
database:sadd("KENAEM:gog"..bot_id, gps[i])
database:set('KENAEM:'..bot_id.."enable:"..gps[i],true)
database:set( 'KENAEM:'..bot_id.."charge:"..gps[i],true)
end
...
local gps2 = database:smembers("KENAEM:gog"..bot_id) or 0
for i=1,#gps do
database:del("KENAEM:gog"..bot_id)
database:del('KENAEM:'..bot_id.."enable:"..gps[i])
database:del( 'KENAEM:'..bot_id.."charge:"..gps[i])
end
...
database:del("KENAEM:gog"..bot_id)
database:del('KENAEM:'..bot_id.."groups")
end
end --
if text:match("^روابط الكروبات$") then
local gpss = database:smembers("KENAEM:"..bot_id.."groups") or 0
local gps = database:scard('KENAEM:'..bot_id.."groups")
text = '📊┇روابط الكروبات\n\n'
for i=1, #gpss do
local link = database:get('KENAEM:'..bot_id.."group:link"..gpss[i])
text = text.."|"..i.."| ~⪼ "..gpss[i].."\n ~⪼ "..(link or  "لا يوجد رابط").."\n"
end
 local f = io.open('KENAEM.txt', 'w')
 f:write(text)
 f:close()
 local KENAEMe = 'https://api.telegram.org/bot' .. token .. '/sendDocument'
 local curl = 'curl "' .. KENAEMe .. '" -F "chat_id=' .. msg.chat_id_ .. '" -F "document=@' .. 'KENAEM.txt' .. '"'
 io.popen(curl)
 end
if text:match("^تحديث$") and tonumber(msg.sender_user_id_) == tonumber(sudo_add) then
send(msg.chat_id_, msg.id_, 1, '☑┇تم التحديث', 1, 'md')
os.execute('rm -rf ./libs/utils.lua')
os.execute('cd libs && wget https://raw.githubusercontent.com/KENAEMTEAM/KENAEM/master/libs/utils.lua')
os.execute('rm -rf KENAEM.lua')
os.execute('wget https://github.com/kkenae/KENAEM/edit/master/KENAE.lua')
os.exit()
return false
end
...
local time = a[2] * day
database:setex( 'KENAEM:'..bot_id.."charge:"..msg.chat_id_,time,true)
database:set( 'KENAEM:'..bot_id.."enable:"..msg.chat_id_,true)
...
local ex = database:ttl( 'KENAEM:'..bot_id.."charge:"..txt[2])
if ex == -1 then
...
database:del("KENAEM:gog"..bot_id,txt[2])
...
database:setex( 'KENAEM:'..bot_id.."charge:"..txt[2],timeplan1,true)
...
database:set( 'KENAEM:'..bot_id.."enable:"..txt[2],true)
end
getUser(msg.sender_user_id_, keko333)
end
...
database:setex( 'KENAEM:'..bot_id.."charge:"..txt[2],timeplan2,true)
...
function keko333(extra,result,success)
keko_info = '@'..(result.username_ or 'لا يوجد')..''
database:set( 'KENAEM:'..bot_id.."charge:"..txt[2],true)
...
local files_KENAEM = database:smembers("files"..bot_id)
local keko = io.popen('cd files_KENAEM && ls'):read("*all")
local files_KENAEM2 = ''
for i=1,#files_KENAEM do
files_KENAEM2 = files_KENAEM2..'{'..files_KENAEM[i]..'}\n'
end
send(msg.chat_id_, msg.id_, 1, '☑┇جميع الملفات : \n '..keko..'\n ---------------------- \n\n✔┇الملفات المفعله \n'..files_KENAEM2..'', 1, 'html')
end
text = msg.content_.text_
if text then
...
local name_t = {string.match(text, "^(حذف ملف) (.*)(.lua)$")}
io.popen("rm -fr files_KENAEM/"..name_t[2]..'.lua')
database:srem("files"..bot_id,name_t[2]..'.lua')
...
local KENAEMe = 'https://api.telegram.org/bot' .. token .. '/sendDocument'
local curl = 'curl "' .. KENAEMe .. '" -F "chat_id=' .. msg.chat_id_ .. '" -F "document=@' .. 'files_KENAEM/'..name_t[2]..'.lua' .. '"'
io.popen(curl)
end
end
end -- end if sudo
...
function KENAEM_run_file(data)
local files_KENAEM = database:smembers("files"..bot_id)
for i=1,#files_KENAEM do
local KENAEMee = dofile("files_KENAEM/"..files_KENAEM[i])
local kt = KENAEMee.keko_KENAEM(data)
if kt == 'end' then
return false
end
end
...
local photo = database:get('KENAEM:'..bot_id..'setphoto'..msg.chat_id_..':'..msg.sender_user_id_)
if photo then
local idPh = nil
...
database:del('KENAEM:'..bot_id..'setphoto'..msg.chat_id_..':'..msg.sender_user_id_)
end end
...
return false end
  ...
  if not database:get( 'KENAEM:'..bot_id.."charge:"..msg.chat_id_) then
  if database:get( 'KENAEM:'..bot_id.."enable:"..msg.chat_id_) then
  database:del( 'KENAEM:'..bot_id.."enable:"..msg.chat_id_)
  for k,v in pairs(sudo_users) do
  end
  end
  end
    ...
    if not database:sismember( 'KENAEM:'..bot_id.."groups",msg.chat_id_) then
    database:sadd( 'KENAEM:'..bot_id.."groups",msg.chat_id_)
    end
    elseif id:match('^(%d+)') then
    database:sadd('KENAEM:'..bot_id.."userss",msg.chat_id_)
    else
    if not database:sismember( 'KENAEM:'..bot_id.."groups",msg.chat_id_) then
    database:sadd( 'KENAEM:'..bot_id.."groups",msg.chat_id_)
    end
  ...
  database:incr('KENAEM:'..bot_id..'user:msgs'..msg.chat_id_..':'..msg.sender_user_id_)
  if database:get('KENAEM:'..bot_id..'viewget'..msg.sender_user_id_) then
  if not msg.forward_info_ then
  send(msg.chat_id_, msg.id_, 1, '❕┇قم بارسال المنشور من القناة', 1, 'md')
  database:del('KENAEM:'..bot_id..'viewget'..msg.sender_user_id_)
  else
  ...
  local hash =  'KENAEM:'..bot_id..'mods:'..msg.chat_id_
  if database:sismember(hash, result.sender_user_id_) then
  ...
  if result.id_ then
  database:sadd('KENAEM:'..bot_id..'mods:'..msg.chat_id_, result.id_)
  texts = '👤┇العضو ~⪼ ['..result.title_..'](t.me/'..(apmd[2] or 'KENAEMteam')..')\n☑┇تم رفعه ادمن'
  else
  texts = '✖┇خطاء'
  end
  send(msg.chat_id_, msg.id_, 1, texts, 1, 'md')
  ...
  database:sadd('KENAEM:'..bot_id..'mods:'..msg.chat_id_, apmd[2])
  ...
  function promote_by_reply(extra, result, success)
  local hash =  'KENAEM:'..bot_id..'vipgp:'..msg.chat_id_
  if database:sismember(hash, result.sender_user_id_) then
tsX000("prore",msg,"☑┇بالفعل تم رفعه عضو مميز") 
  ...
  function promote_by_username(extra, result, success)
  if result.id_ then
  database:sadd('KENAEM:'..bot_id..'vipgp:'..msg.chat_id_, result.id_)
  texts = '👤┇العضو ~⪼ ['..result.title_..'](t.me/'..(apmd[2] or 'KENAEMteam')..')\n☑┇تم رفعه عضو مميز'
  else
  texts = '✖┇خطاء'
  end
  ...
  database:sadd('KENAEM:'..bot_id..'vipgp:'..msg.chat_id_, apmd[2])
  ...
  database:set("tsahke:set:id:"..bot_id..msg.chat_id_..msg.sender_user_id_,'KENAEM')
  return "KENAEM"
  end
  ...
  database:set("KENAEM:gr:id:text:"..bot_id..msg.chat_id_,text)
  end
  if text and text == "مسح الايدي" and is_owner(msg) then
  send(msg.chat_id_, msg.id_, 1,  '✖┇ تم المسح بنجاح', 1, 'md')
  database:del("KENAEM:gr:id:text:"..bot_id..msg.chat_id_)
  end
  ...
  tsX000(apmd[2],msg,"☑┇تم تنزيله من اعضاء الممزين البوت")
    ...
    local hash =  'KENAEM:'..bot_id..'banned:'..msg.chat_id_
 ...
 chat_kick(result.chat_id_, result.sender_user_id_)
  ...
  send(msg.chat_id_, msg.id_, 1, '❕┇لا تستطيع حظر \n🔘┇(مدراء،ادمنيه،اعضاء مميزين)البوت', 1, 'md')
      ...
      database:sadd('KENAEM:'..bot_id..'banned:'..msg.chat_id_, result.id_)
      texts = '👤┇العضو ~⪼ ['..result.title_..'](t.me/'..(apba[2] or 'KENAEMteam')..')\n☑┇تم حظره من المجموعه'
  ...
  send(msg.chat_id_, msg.id_, 1, '❕┇لا تستطيع حظر \n🔘┇(مدراء،ادمنيه،اعضاء مميزين)البوت', 1, 'md')
      ...
      database:sadd('KENAEM:'..bot_id..'banned:'..msg.chat_id_, apba[2])
  ...
  if database:get('KENAEM:'..bot_id.."group:link"..msg.chat_id_) == 'Waiting For Link!\nPls Send Group Link' and is_mod(msg) then 
  ...
  local hash = 'KENAEM:'..bot_id.."group:link"..msg.chat_id_ 
  ...
  send(msg.chat_id_, 0, 1, '↙️┇رابط المجموعه الجديد\n'..glink, 1, 'html')
   ...
   if result.id_ then
     database:srem('KENAEM:'..bot_id..'banned:'..msg.chat_id_, result.id_)
  texts = '👤┇العضو ~⪼ ['..result.title_..'](t.me/'..(apba[2] or 'KENAEMteam')..')\n☑┇تم الغاء حظره من البوت' 
   ...
   local apba = {string.match(text, "^(الغاء حظر) (%d+)$")}
     database:srem('KENAEM:'..bot_id..'banned:'..msg.chat_id_, apba[2])
  ...
  function gban_by_reply(extra, result, success)
    local hash =  'KENAEM:'..bot_id..'gbanned:'
  ...
  send(msg.chat_id_, msg.id_, 1, '❕┇لا تستطيع حظر عام \n🔘┇(مدراء،ادمنيه،اعضاء مميزين)البوت', 1, 'md')
    ...
    local hash =  'KENAEM:'..bot_id..'gbanned:'
  texts = '👤┇العضو ~⪼ ['..result.title_..'](t.me/'..(apbll[2] or 'KENAEMteam')..')\n🚫┇تم حظره من المجموعات البوت'
  ...
  function ungban_by_username(extra, result, success)
    local hash =  'KENAEM:'..bot_id..'gbanned:'
    if result.id_ then
  texts = '👤┇العضو ~⪼ ['..result.title_..'](t.me/'..(apid[2] or 'KENAEMteam')..')\n🚫┇تم الغاء حظره من المجموعات البوت'
  database:srem(hash, result.id_)
    else
    texts = '✖┇خطاء'
    end
  ...
  function gmute_by_reply(extra, result, success)
    local hash =  'KENAEM:'..bot_id..'gmuted:'
    if is_admin(result) then
...
tsX000("prore",msg,"🚫┇تم الغاء كتمه من المجموعات البوت")
  ...
  tsX000(apbll[2],msg,"🚫┇تم الغاء كتمه من المجموعات البوت")
    ...
    local hash =  'KENAEM:'..bot_id..'muted:'..msg.chat_id_
    if ck_mod(result.sender_user_id_, msg.chat_id_) then
...
tsX000("prore",msg,"🚫┇بالفعل تم كتمه")
  ...
  send(msg.chat_id_, msg.id_, 1, '✖┇لا تستطيع كتم \n🔘┇(مدراء،ادمنيه،اعضاء مميزين)البوت', 1, 'md')
      ...
      database:sadd('KENAEM:'..bot_id..'muted:'..msg.chat_id_, result.id_)
    texts = '👤┇العضو ~⪼ ['..result.title_..'](t.me/'..(apsi[2] or 'KENAEMteam')..')\n🚫┇تم كتمه من البوت'
    end
  else
    texts = '✖┇خطاء'
  end
...
database:sadd('KENAEM:'..bot_id..'muted:'..msg.chat_id_, apsi[2])
tsX000(apsi[2],msg,"🚫┇تم كتمه من البوت")
    ...
    if result.id_ then
     database:srem('KENAEM:'..bot_id..'muted:'..msg.chat_id_, result.id_)
   texts = '👤┇العضو ~⪼ ['..result.title_..'](t.me/'..(apsi[2] or 'KENAEMteam')..')\n🚫┇تم الغاء كتمه من البوت'
    ...
    local apsi = {string.match(text, "^(الغاء كتم) (%d+)$")}
      database:srem('KENAEM:'..bot_id..'muted:'..msg.chat_id_, apsi[2])
  ...
  send(msg.chat_id_, msg.id_, 1, '✖┇لا تستطيع طرد \n🔘┇(مدراء،ادمنيه،اعضاء مميزين)البوت', 1, 'md')
    ...
    texts = '👤┇العضو ~⪼ ['..result.title_..'](t.me/'..(apki[2] or 'KENAEMteam')..')\n🚫┇تم طرده من المجموعه'
       chat_kick(msg.chat_id_, result.id_)
    end
  else
    texts = '✖┇خطاء'
  ...
  send(msg.chat_id_, msg.id_, 1, '✖┇لا تستطيع طرد \n🔘┇(مدراء،ادمنيه،اعضاء مميزين)البوت', 1, 'md')
    ...
    local hash =  'KENAEM:'..bot_id..'owners:'..msg.chat_id_
    if database:sismember(hash, result.sender_user_id_) then
    ...
    if result.id_ then
      database:sadd('KENAEM:'..bot_id..'owners:'..msg.chat_id_, result.id_)
texts = '👤┇العضو ~⪼ ['..result.title_..'](t.me/'..(apow[2] or 'KENAEMteam')..')\n☑┇تم رفع مدير في البوت'
    ...
    local apow = {string.match(text, "^(رفع مدير) (%d+)$")}
      database:sadd('KENAEM:'..bot_id..'owners:'..msg.chat_id_, apow[2])
  ...
  if text:match("^جلب صوره (%d+)$") and msg.reply_to_message_id_ == 0 and not database:get('KENAEM:'..bot_id..'get:photo'..msg.chat_id_) then
...
local ph = pronumb[2] - 1
  ...
  database:set('KENAEM:'..bot_id..'flood:max:'..msg.chat_id_,floodmax[2])
  ...
  database:set('KENAEM:'..bot_id..'flood:time:'..msg.chat_id_,floodt[2])
    ...
    local welcome = {string.match(text, "^(وضع ترحيب) (.*)$")}
     send(msg.chat_id_, msg.id_, 1, '☑┇تم وضع ترحيب\n📜┇~⪼('..welcome[2]..')', 1, 'md')
       database:set('KENAEM:'..bot_id..'welcome:'..msg.chat_id_,welcome[2])
    end
    ...
    local wel = database:get('KENAEM:'..bot_id..'welcome:'..msg.chat_id_)
    if wel then
  ...
  database:hset('KENAEM:'..bot_id..'filters:'..msg.chat_id_, name, 'filtered')
  ...
  database:hdel('KENAEM:'..bot_id..'filters:'..msg.chat_id_, rws[2])
  ...
  if (text:match("^كشف البوتات$") or text:match("^البوتات$")) and is_mod(msg) then
   ...
   n = (n + 1)
     local user_info = database:hgetall('KENAEM:'..bot_id..'user:'..v.user_id_)
  if user_info and user_info.username then
   local username = user_info.username
...
text = ""
  ...
  local user_msgs = database:get('KENAEM:'..bot_id..'user:msgs'..msg.chat_id_..':'..msg.sender_user_id_)
   if not database:get('KENAEM:'..bot_id..'id:mute'..msg.chat_id_) then
...
add = (tonumber(database:get('KENAEM:'..bot_id..'user:add'..msg.chat_id_..':'..msg.sender_user_id_)) or 0)
  send(msg.chat_id_, msg.id_, 1, "📨┇عدد اضافه جهاتك ~⪼ *{"..add.."}*\n📨┇سيتم حذف العدد بعد هذه الرساله", 1, 'md')
database:del('KENAEM:'..bot_id..'user:add'..msg.chat_id_..':'..msg.sender_user_id_)
  end
  if text:match("^(عدد السحكات)$") or text:match("^(سحكاتي)$") then
  local edit = database:get('KENAEM:'..bot_id..'user:editmsg'..msg.chat_id_..':'..msg.sender_user_id_) or 0
  ...
  database:del('KENAEM:'..bot_id..'banned:'..msg.chat_id_)
  ...
  database:del('KENAEM:'..bot_id..'creator:'..msg.chat_id_)
...
text = ""
  ...
  send(msg.chat_id_, msg.id_, 1, moody, 1, 'html')
   ...
   bot.channel_get_admins(msg.chat_id_,cb)
    ...
    if database:get("lock_media:KENAEM"..msg.chat_id_..bot_id) then
       mute_all = '✔┇'
       else
       mute_all = '✖┇'
       end
       ...
       ------------
       if not database:get('KENAEM:'..bot_id..'flood:max:'..msg.chat_id_) then
       flood_m = 10
       else
       flood_m = database:get('KENAEM:'..bot_id..'flood:max:'..msg.chat_id_)
     end
       if not database:get( 'KENAEM:'..bot_id..'flood:time:'..msg.chat_id_) then
       flood_t = 10
       else
       flood_t = database:get( 'KENAEM:'..bot_id..'flood:time:'..msg.chat_id_)
     end
       ------------
       if database:get("lock_audeo:KENAEM"..msg.chat_id_..bot_id) then
       mute_music = '✔┇'
       ...
       ------------
       if database:get("lock_inline:KENAEM"..msg.chat_id_..bot_id) then
       mute_in = '✔┇'
       else
       mute_in = '✖┇'
       end
       ...
       ------------
       if database:get("lock_edit:KENAEM"..msg.chat_id_..bot_id) then
       mute_edit = '✔┇'
       else
       mute_edit = '✖┇'
       end
   ...
   ------------
       if database:get("lock_pin:KENAEM"..msg.chat_id_..bot_id) then
       lock_pin = '✔┇'
       else
       lock_pin = '✖┇'
     end
  ...
  if database:get("lock_contact:KENAEM"..msg.chat_id_..bot_id) then
   ...
   if database:get('KENAEM:'..bot_id..'rep:mute'..msg.chat_id_) then
       lock_rep = '✔┇'
       else
       lock_rep = '✖┇'
       end
       ------------
   if database:get('KENAEM:'..bot_id..'repsudo:mute'..msg.chat_id_) then
       lock_repsudo = '✔┇'
       else
       lock_repsudo = '✖┇'
       end
       ------------
   if database:get('KENAEM:'..bot_id..'repowner:mute'..msg.chat_id_) then
       lock_repowner = '✔┇'
       else
       lock_repowner = '✖┇'
       end
       ------------
   if database:get('KENAEM:'..bot_id..'id:mute'..msg.chat_id_) then
       lock_id = '✔┇'
       else
       lock_id = '✖┇'
       end
       ------------
   if database:get('KENAEM:'..bot_id..'pin:mute'..msg.chat_id_) then
       lock_pind = '✔┇'
       else
       lock_pind = '✖┇'
       end
   ...
   if database:get('KENAEM:'..bot_id..'id:mute'..msg.chat_id_) then
       ...
       ------------
if database:get( 'KENAEM:'..bot_id.."welcome"..msg.chat_id_) then
send_welcome = '✔┇'
else
send_welcome = '✖┇'
end
    ------------
local ex = database:ttl( 'KENAEM:'..bot_id.."charge:"..msg.chat_id_)
  if ex == -1 then
  exp_dat = 'لا نهائي'
  else
  ...
  if (text and text == 'تفعيل اطردني') and is_owner(msg) then
     if not database:get('KENAEM:'..bot_id..'kickme:mute'..msg.chat_id_) then
   send(msg.chat_id_, msg.id_, 1, '☑┇امر اطردني بالفعل تم تفعيله', 1, 'md')
     else
   send(msg.chat_id_, msg.id_, 1, '☑┇تم تفعيل امر اطردني', 1, 'md')
    database:del('KENAEM:'..bot_id..'kickme:mute'..msg.chat_id_)
   end
   end
   if (text and text == 'تعطيل اطردني') and is_owner(msg) then
     if database:get('KENAEM:'..bot_id..'kickme:mute'..msg.chat_id_) then
   send(msg.chat_id_, msg.id_, 1, '☑┇امر اطردني بالفعل تم تعطيله', 1, 'md')
   else
   send(msg.chat_id_, msg.id_, 1, '☑┇تم تعطيل امر اطردني', 1, 'md')
     database:set('KENAEM:'..bot_id..'kickme:mute'..msg.chat_id_,true)
   end
     end

  if text:match("^اطردني$") then
  if not database:get('KENAEM:'..bot_id..'kickme:mute'..msg.chat_id_) then
  redis:set('KENAEM:'..bot_id..'kickyess'..msg.sender_user_id_..'', 'kickyes')
  redis:set('KENAEM:'..bot_id..'kicknoo'..msg.sender_user_id_..'', 'kickno')
  send(msg.chat_id_, msg.id_, 1, '🚷┇ ارسل ؛ نعم ، ليتم طردك\n🔘┇ارسل ؛ لا ، لالغاء الامر', 1, 'md')
  else
  send(msg.chat_id_, msg.id_, 1, '⚠┇تم تعطيل امر اطردني', 1, 'md')
  end
  end
  local yess = redis:get('KENAEM:'..bot_id..'kickyess'..msg.sender_user_id_..'')
  ...
  if yess == 'kickyes' then
  chat_kick(msg.chat_id_, msg.sender_user_id_)
  redis:del('KENAEM:'..bot_id..'kickyess'..msg.sender_user_id_..'', 'kickyes')
  redis:del('KENAEM:'..bot_id..'kicknoo'..msg.sender_user_id_..'', 'kickno')
  send(msg.chat_id_, msg.id_, 1, '✅┇تم طردك من المجموعه', 1, 'md')
    end
    end
  end
  if text:match("^لا$") then
  local noo = redis:get('KENAEM:'..bot_id..'kicknoo'..msg.sender_user_id_..'')
  if noo == 'kickno' then
  redis:del('KENAEM:'..bot_id..'kickyess'..msg.sender_user_id_..'', 'kickyes')
  redis:del('KENAEM:'..bot_id..'kicknoo'..msg.sender_user_id_..'', 'kickno')
  ...
  redis:set('KENAEM:'..bot_id..'texts'..msg.sender_user_id_..'', 'msg')
    return false end
  if text:match("^(.*)$") then
  local keko2 = redis:get('KENAEM:'..bot_id..'texts'..msg.sender_user_id_..'')
  if keko2 == 'msg' then
  send(msg.chat_id_, msg.id_, 1, '☑┇تم حفظ الكليشه يمكنك اظهارها بارسال الامر', 1, 'html')
  redis:set('KENAEM:'..bot_id..'texts'..msg.sender_user_id_..'', 'no')
  redis:set('KENAEM:'..bot_id..'text_sudo', text)
  send(msg.chat_id_, msg.id_, 1, text , 1, 'html')
    return false end
   end
  if  (text and text == 'مسح امر المطور بالكليشه') and tonumber(msg.sender_user_id_) == tonumber(sudo_add) then
   redis:del('KENAEM:'..bot_id..'text_sudo', text)
  ...
  if text:match("^[Dd][Ee][Vv]$")or text:match("^مطور بوت$") or text:match("^مطورين$") or text:match("^مطور البوت$") or text:match("^مطور$") or text:match("^المطور$") and msg.reply_to_message_id_ == 0 then
    local text_sudo = redis:get('KENAEM:'..bot_id..'text_sudo')
  local nkeko = redis:get('KENAEM:'..bot_id..'nmkeko')
  local nakeko = redis:get('KENAEM:'..bot_id..'nakeko')
    if text_sudo then
    send(msg.chat_id_, msg.id_, 1, text_sudo, 1, 'md')
    else
    sendContact(msg.chat_id_, msg.id_, 0, 1, nil, (nkeko or 9647707641864), (nakeko or "KENAEM TEAM"), "", bot_id)
  end
   end
    for k,v in pairs(sudo_users) do
  ...
  redis:set('KENAEM:'..bot_id..'nkeko'..msg.sender_user_id_..'', 'msg')
    return false end
  end
  if text:match("^+(.*)$") then
  local kekoo = redis:get('KENAEM:'..bot_id..'sudoo'..text..'')
  local keko2 = redis:get('KENAEM:'..bot_id..'nkeko'..msg.sender_user_id_..'')
  if keko2 == 'msg' then
  send(msg.chat_id_, msg.id_, 1, '• `الان يمكنك ارسال الاسم الذي تريده` 🏷', 1, 'md')
  redis:set('KENAEM:'..bot_id..'nmkeko', text)
  redis:set('KENAEM:'..bot_id..'nkeko'..msg.sender_user_id_..'', 'mmsg')
    return false end
  end
  if text:match("^(.*)$") then
  local keko2 = redis:get('KENAEM:'..bot_id..'nkeko'..msg.sender_user_id_..'')
  ...
  redis:set('KENAEM:'..bot_id..'nkeko'..msg.sender_user_id_..'', 'no')
  redis:set('KENAEM:'..bot_id..'nakeko', text)
  local nmkeko = redis:get('KENAEM:'..bot_id..'nmkeko')
  sendContact(msg.chat_id_, msg.id_, 0, 1, nil, nmkeko, text , "", bot_id)
    ...
    function promote_by_reply(extra, result, success)
    if redis:sismember('KENAEM:'..bot_id..'dev', result.sender_user_id_) then
	tsX000("prore",msg,'☑┇بالفعل تم رفعه مطور')
  else
    redis:set('KENAEM:'..bot_id..'sudoo'..result.sender_user_id_..'', 'yes')
    redis:sadd('KENAEM:'..bot_id..'dev', result.sender_user_id_)
    ...
    function promote_by_username(extra, result, success)
    if result.id_ then
    redis:set('KENAEM:'..bot_id..'sudoo'..result.id_..'', 'yes')
    redis:sadd('KENAEM:'..bot_id..'dev', result.id_)
      texts = '👤┇العضو ~⪼ ['..result.title_..'](t.me/'..(apmd[2] or 'KENAEMteam')..')\n\n☑┇تم رفعه مطور'
  else
    texts = '✖┇خطاء'
  end
    ...
    redis:set('KENAEM:'..bot_id..'sudoo'..apmd[2]..'', 'yes')
    redis:sadd('KENAEM:'..bot_id..'dev', apmd[2])
    ...
    function demote_by_reply(extra, result, success)
    if not redis:sismember('KENAEM:'..bot_id..'dev', result.sender_user_id_) then
	tsX000("prore",msg,'☑┇ بالفعل تم تنزيله من المطورين')
    else
    redis:del('KENAEM:'..bot_id..'sudoo'..result.sender_user_id_..'', 'no')
    redis:srem('KENAEM:'..bot_id..'dev', result.sender_user_id_)
    ...
    function demote_by_username(extra, result, success)
    if result.id_ then
    redis:del('KENAEM:'..bot_id..'sudoo'..result.id_..'', 'no')
    redis:srem('KENAEM:'..bot_id..'dev', result.id_)
    ...
    redis:del('KENAEM:'..bot_id..'sudoo'..apmd[2]..'', 'no')
    redis:srem('KENAEM:'..bot_id..'dev', apmd[2])
  ...
  redis:set('KENAEM:'..bot_id..'keko1'..msg.sender_user_id_..''..msg.chat_id_..'', 'msg')
    return false end
  if text:match("^(.*)$") then
  if not database:get('KENAEM:'..bot_id..'repowner:mute'..msg.chat_id_) then
  local keko = redis:get('KENAEM:'..bot_id..'keko'..text..''..msg.chat_id_..'')
  send(msg.chat_id_, msg.id_, 1, keko, 1, 'md')
  end
  local keko1 = redis:get('KENAEM:'..bot_id..'keko1'..msg.sender_user_id_..''..msg.chat_id_..'')
  if keko1 == 'msg' then
  send(msg.chat_id_, msg.id_, 1, '📥┇الان ارسل الرد الذي تريد اضافته', 1, 'md')
  redis:set('KENAEM:'..bot_id..'keko1'..msg.sender_user_id_..''..msg.chat_id_..'', 're')
  redis:set('KENAEM:'..bot_id..'msg'..msg.sender_user_id_..''..msg.chat_id_..'', text)
  redis:sadd('KENAEM:'..bot_id..'repowner'..msg.sender_user_id_..''..msg.chat_id_..'', text)
    return false end
  if keko1 == 're' then
  local keko2 = redis:get('KENAEM:'..bot_id..'msg'..msg.sender_user_id_..''..msg.chat_id_..'')
  redis:set('KENAEM:'..bot_id..'keko'..keko2..''..msg.chat_id_..'', text)
  redis:sadd('KENAEM:'..bot_id..'kekore'..msg.chat_id_..'', keko2)
  send(msg.chat_id_, msg.id_, 1, "☑┇تم حفظ الرد", 1, 'md')
  redis:set('KENAEM:'..bot_id..'keko1'..msg.sender_user_id_..''..msg.chat_id_..'', 'no')
  end
  end
    if text:match("^حذف رد$") and is_owner(msg) then
  send(msg.chat_id_, msg.id_, 1, '📥┇ارسل الكلمه التي تريد حذفها', 1, 'md')
  redis:set('KENAEM:'..bot_id..'keko1'..msg.sender_user_id_..''..msg.chat_id_..'', 'nomsg')
    return false end
  if text:match("^(.*)$") then
  local keko1 = redis:get('KENAEM:'..bot_id..'keko1'..msg.sender_user_id_..''..msg.chat_id_..'')
  if keko1 == 'nomsg' then
  ...
  redis:set('KENAEM:'..bot_id..'keko1'..msg.sender_user_id_..''..msg.chat_id_..'', 'no')
  redis:set('KENAEM:'..bot_id..'keko'..text..''..msg.chat_id_..'', " ")
 redis:srem('KENAEM:'..bot_id..'kekore'..msg.chat_id_..'', text)
   end
  end
  ...
  redis:set('KENAEM:'..bot_id..'keko1'..msg.sender_user_id_..'', 'msg')
    return false end
  if text:match("^(.*)$") then
  if not database:get('KENAEM:'..bot_id..'repsudo:mute'..msg.chat_id_) then
  local keko = redis:get('KENAEM:'..bot_id..'keko'..text..'')
  send(msg.chat_id_, msg.id_, 1, keko, 1, 'md')
  end
  local keko1 = redis:get('KENAEM:'..bot_id..'keko1'..msg.sender_user_id_..'')
  if keko1 == 'msg' then
  send(msg.chat_id_, msg.id_, 1, '📥┇الان ارسل الرد الذي تريد اضافته', 1, 'md')
  redis:set('KENAEM:'..bot_id..'keko1'..msg.sender_user_id_..'', 're')
  redis:set('KENAEM:'..bot_id..'msg'..msg.sender_user_id_..'', text)
    return false end
  if keko1 == 're' then
  local keko2 = redis:get('KENAEM:'..bot_id..'msg'..msg.sender_user_id_..'')
  redis:set('KENAEM:'..bot_id..'keko'..keko2..'', text)
  redis:sadd('KENAEM:'..bot_id..'kekoresudo', keko2)
  send(msg.chat_id_, msg.id_, 1, "☑┇تم حفظ الرد", 1, 'md')
  redis:set('KENAEM:'..bot_id..'keko1'..msg.sender_user_id_..'', 'no')
  end
  end

  if text:match("^حذف رد للكل$") and tonumber(msg.sender_user_id_) == tonumber(sudo_add)  then
  send(msg.chat_id_, msg.id_, 1, '📥┇ارسل الكلمه التي تريد حذفها' , 1, 'md')
  redis:set('KENAEM:'..bot_id..'keko1'..msg.sender_user_id_..'', 'nomsg')
    return false end
  if text:match("^(.*)$") then
  local keko1 = redis:get('KENAEM:'..bot_id..'keko1'..msg.sender_user_id_..'')
  if keko1 == 'nomsg' then
  send(msg.chat_id_, msg.id_, 1, '☑┇تم حذف الرد', 1, 'md')
  redis:set('KENAEM:'..bot_id..'keko1'..msg.sender_user_id_..'', 'no')
   redis:set('KENAEM:'..bot_id..'keko'..text..'', " ")
   redis:srem('KENAEM:'..bot_id..'kekoresudo', text)
   end
  end

  if text:match("^مسح المطورين$") and tonumber(msg.sender_user_id_) == tonumber(sudo_add) then
    local list = redis:smembers('KENAEM:'..bot_id..'dev')
    for k,v in pairs(list) do
  redis:del('KENAEM:'..bot_id..'dev', text)
  redis:del('KENAEM:'..bot_id..'sudoo'..v..'', 'no')
  end
    send(msg.chat_id_, msg.id_, 1, "☑┇تم مسح مطورين البوت", 1, 'md')
    end
    if text:match("^مسح ردود المدير$") and is_owner(msg) then
    local list = redis:smembers('KENAEM:'..bot_id..'kekore'..msg.chat_id_..'')
    for k,v in pairs(list) do
  redis:del('KENAEM:'..bot_id..'kekore'..msg.chat_id_..'', text)
  redis:set('KENAEM:'..bot_id..'keko'..v..''..msg.chat_id_..'', " ")
  end
    ...
    local list = redis:smembers('KENAEM:'..bot_id..'kekoresudo')
    for k,v in pairs(list) do
  redis:del('KENAEM:'..bot_id..'kekoresudo', text)
  redis:set('KENAEM:'..bot_id..'keko'..v..'', " ")
  end
  ...
  text = "❕┇لا يوجد ردود للمدير"
    ...
    database:set('KENAEM:'..bot_id..'rules'..msg.chat_id_, txt[2])
     send(msg.chat_id_, msg.id_, 1, "✔┇تم وضع القوانين للمجموعه", 1, 'md')
    end
    
      if  text:match("^القوانين$") then
    local rules = database:get('KENAEM:'..bot_id..'rules'..msg.chat_id_)
    if rules then
  ...
  database:set('KENAEM:'..bot_id..'setphoto'..msg.chat_id_..':'..msg.sender_user_id_,true)
...
if (text and text == 'تفعيل جلب صوره') and is_owner(msg) then
  ...
  if database:sismember('KENAEM:'..bot_id..'pro:groups',msg.chat_id_) then
    ...
    send(msg.chat_id_, msg.id_, 1, '☑┇تم اضافه المجموعه الى الوضع المدفوع', 1, 'md')
     database:sadd('KENAEM:'..bot_id..'pro:groups',msg.chat_id_)
  end
  end
  if   (text and text == 'تعطيل وضع المدفوع') and tonumber(msg.sender_user_id_) == tonumber(sudo_add) then
  if not database:sismember('KENAEM:'..bot_id..'pro:groups',msg.chat_id_) then
  ...
  database:srem('KENAEM:'..bot_id..'pro:groups',msg.chat_id_)
  end
  end
  ...
  send(msg.chat_id_, msg.id_, 1, '☑┇تم تفعيل ردود المطور', 1, 'md')
   database:del('KENAEM:'..bot_id..'repsudo:mute'..msg.chat_id_)
  end
  end
  ...
  send(msg.chat_id_, msg.id_, 1, 'تم تعطيل ردود المطور', 1, 'md')
    database:set('KENAEM:'..bot_id..'repsudo:mute'..msg.chat_id_,true)
  end
    end
  ...
  send(msg.chat_id_, msg.id_, 1, '☑┇تم تفعيل ردود المدير', 1, 'md')
   database:del('KENAEM:'..bot_id..'repowner:mute'..msg.chat_id_)
  end
  end
  ...
  send(msg.chat_id_, msg.id_, 1, '☑┇تم تعطيل ردود المدير', 1, 'md')
    database:set('KENAEM:'..bot_id..'repowner:mute'..msg.chat_id_,true)
  end
    end
  ...
  send(msg.chat_id_, msg.id_, 1, '☑┇تم تفعيل الايدي', 1, 'md')
   database:del('KENAEM:'..bot_id..'id:mute'..msg.chat_id_)
  end
  end
  ...
  send(msg.chat_id_, msg.id_, 1, '☑┇تم تعطيل الايدي', 1, 'md')
    database:set('KENAEM:'..bot_id..'id:mute'..msg.chat_id_,true)
  end
  end
  ...
  database:set("KENAEM"..bot_id..":sudo:gr",msg.chat_id_)
  end
  ...
  database:del("KENAEM"..bot_id..":sudo:gr")
  end
  if text:match("^ايدي$") and msg.reply_to_message_id_ == 0 then
  local keko_info = nil
  function keko333(extra,result,success)
  ...
  if not database:get('KENAEM:'..bot_id..'id:mute'..msg.chat_id_) then
  if not database:get('KENAEM:'..bot_id..'id:photo'..msg.chat_id_) then
  if not database:get("KENAEM:gr:id:text:"..bot_id..msg.chat_id_) then 
  local keko_text = {
 ' ياحلو مين الله جابك🤧',
  " غير صورتك 😪 ",
  " تخليني♥️ ",
...
keko3 = math.random(#keko_text)
  ...
  local KENAEM_new_text = database:get("KENAEM:gr:id:text:"..bot_id..msg.chat_id_)
  local KENAEM_new_text = KENAEM_new_text:gsub('#username',(keko_info or 'لا يوجد'))
  local KENAEM_new_text = KENAEM_new_text:gsub('#id',(msg.sender_user_id_ or 'لا يوجد'))
  local KENAEM_new_text = KENAEM_new_text:gsub('#edit',(edit or 'لا يوجد'))
  local KENAEM_new_text = KENAEM_new_text:gsub('#msgs',(user_msgs or 'لا يوجد'))
  local KENAEM_new_text = KENAEM_new_text:gsub('#stast',(t or 'لا يوجد'))
  sendPhoto(msg.chat_id_, msg.id_, 0, 1, nil, result.photos_[0].sizes_[1].photo_.persistent_id_,KENAEM_new_text,msg.id_,msg.id_.."")
  end
  else
  if is_sudo(msg) then
  t = 'مطور البوت'
  ...
  local KENAEM_new_text = KENAEM_new_text:gsub('#msgs',(user_msgs or 'لا يوجد'))
  local KENAEM_new_text = KENAEM_new_text:gsub('#stast',(t or 'لا يوجد'))   
  send(msg.chat_id_, msg.id_, 1, KENAEM_new_text, 1, 'html')
  end   
  end
  else
  end
  ...
  if  (text and text == 'تفعيل التثبيت') and is_owner(msg) then
     if not database:get('KENAEM:'..bot_id..'pin:mute'..msg.chat_id_) then
   send(msg.chat_id_, msg.id_, 1, '☑┇التثبيت بالفعل تم تفعيله', 1, 'md')
     else
   send(msg.chat_id_, msg.id_, 1, '☑┇تم تفعيل التثبيت', 1, 'md')
    database:del('KENAEM:'..bot_id..'pin:mute'..msg.chat_id_)
   end
   end
   ...
   send(msg.chat_id_, msg.id_, 1, '☑┇تم تعطيل التثبيت', 1, 'md')
     database:set('KENAEM:'..bot_id..'pin:mute'..msg.chat_id_,true)
   end
     end
   ...
   pin(msg.chat_id_,msg.reply_to_message_id_,0)
       database:set('KENAEM:'..bot_id..'pinnedmsg'..msg.chat_id_,msg.reply_to_message_id_)
    ...
    database:set('KENAEM:'..bot_id..'viewget'..msg.sender_user_id_,true)
  ...
  redis:del('KENAEM:'..bot_id..'help', text)
  redis:del('KENAEM:'..bot_id..'h1', text)
  redis:del('KENAEM:'..bot_id..'h2', text)
  redis:del('KENAEM:'..bot_id..'h3', text)
  redis:del('KENAEM:'..bot_id..'h4', text)
  redis:del('KENAEM:'..bot_id..'h5', text)
  redis:del('KENAEM:'..bot_id..'h6', text)
  ...
  send(msg.chat_id_, msg.id_, 1, '📥┇الان يمكنك ارسال الكليشه  ليتم حفظها', 1, 'html')
  redis:set('KENAEM:'..bot_id..'hhh'..msg.sender_user_id_..'', 'msg')
    return false end
  if text:match("^(.*)$") then
  local keko2 = redis:get('KENAEM:'..bot_id..'hhh'..msg.sender_user_id_..'')
  if keko2 == 'msg' then
  send(msg.chat_id_, msg.id_, 1, '☑┇تم حفظ الكليشه يمكنك اظهارها بارسال الامر', 1, 'html')
  redis:set('KENAEM:'..bot_id..'hhh'..msg.sender_user_id_..'', 'no')
  redis:set('KENAEM:'..bot_id..'help', text)
  send(msg.chat_id_, msg.id_, 1, text , 1, 'html')
    return false end
   end

     if text:match("^الاوامر$") and is_mod(msg) then
  local help = redis:get('KENAEM:'..bot_id..'help')
...
🎖┇م4 ~⪼ لعرض اوامر المطورين
 ┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉]]
  ...
  send(msg.chat_id_, msg.id_, 1, '📥┇الان يمكنك ارسال الكليشه  ليتم حفظها', 1, 'html')
  redis:set('KENAEM:'..bot_id..'h11'..msg.sender_user_id_..'', 'msg')
    return false end
  if text:match("^(.*)$") then
  local keko2 = redis:get('KENAEM:'..bot_id..'h11'..msg.sender_user_id_..'')
  if keko2 == 'msg' then
  send(msg.chat_id_, msg.id_, 1, '☑┇تم حفظ الكليشه يمكنك اظهارها بارسال الامر', 1, 'html')
  redis:set('KENAEM:'..bot_id..'h11'..msg.sender_user_id_..'', 'no')
  redis:set('KENAEM:'..bot_id..'h1', text)
  send(msg.chat_id_, msg.id_, 1, text , 1, 'html')
    return false end
   end
     if text:match("^م1$") and is_mod(msg) then
  local h1 = redis:get('KENAEM:'..bot_id..'h1')
     local text =  [[
📮┇ اوامر حمايه المجموعه بالمسح
  ...
  if (text and text == 'تغير امر م2') and tonumber(msg.sender_user_id_) == tonumber(sudo_add) then
   send(msg.chat_id_, msg.id_, 1, '📥┇الان يمكنك ارسال الكليشه  ليتم حفظها', 1, 'html')
   redis:set('KENAEM:'..bot_id..'h22'..msg.sender_user_id_..'', 'msg')
     return false end
   if text:match("^(.*)$") then
   local keko2 = redis:get('KENAEM:'..bot_id..'h22'..msg.sender_user_id_..'')
   if keko2 == 'msg' then
   send(msg.chat_id_, msg.id_, 1, '☑┇تم حفظ الكليشه يمكنك اظهارها بارسال الامر', 1, 'html')
   redis:set('KENAEM:'..bot_id..'h22'..msg.sender_user_id_..'', 'no')
   redis:set('KENAEM:'..bot_id..'h2', text)
   send(msg.chat_id_, msg.id_, 1, text , 1, 'html')
     return false end
    end
     if text:match("^م2$") and is_mod(msg) then
   local h2 = redis:get('KENAEM:'..bot_id..'h2')
...
📜┇رسائلي			
  ...
  if (text and text == 'تغير امر م3') and tonumber(msg.sender_user_id_) == tonumber(sudo_add) then
   send(msg.chat_id_, msg.id_, 1, '📥┇الان يمكنك ارسال الكليشه  ليتم حفظها', 1, 'html')
   redis:set('KENAEM:'..bot_id..'h33'..msg.sender_user_id_..'', 'msg')
     return false end
   if text:match("^(.*)$") then
   local keko2 = redis:get('KENAEM:'..bot_id..'h33'..msg.sender_user_id_..'')
   if keko2 == 'msg' then
   send(msg.chat_id_, msg.id_, 1, '☑┇تم حفظ الكليشه يمكنك اظهارها بارسال الامر', 1, 'html')
   redis:set('KENAEM:'..bot_id..'h33'..msg.sender_user_id_..'', 'no')
   redis:set('KENAEM:'..bot_id..'h3', text)
   send(msg.chat_id_, msg.id_, 1, text , 1, 'html')
     return false end
    end
  if text:match("^م3$") and is_mod(msg) then
  local h4 = redis:get('KENAEM:'..bot_id..'h3')
...
┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉
  ...
  if (text and text == 'تغير امر م4') and tonumber(msg.sender_user_id_) == tonumber(sudo_add) then
    send(msg.chat_id_, msg.id_, 1, '📥┇الان يمكنك ارسال الكليشه  ليتم حفظها', 1, 'html')
    redis:set('KENAEM:'..bot_id..'h44'..msg.sender_user_id_..'', 'msg')
  return false end
    if text:match("^(.*)$") then
    local keko2 = redis:get('KENAEM:'..bot_id..'h44'..msg.sender_user_id_..'')
    if keko2 == 'msg' then
    send(msg.chat_id_, msg.id_, 1, '☑┇تم حفظ الكليشه يمكنك اظهارها بارسال الامر', 1, 'html')
    redis:set('KENAEM:'..bot_id..'h44'..msg.sender_user_id_..'', 'no')
    redis:set('KENAEM:'..bot_id..'h4', text)
    send(msg.chat_id_, msg.id_, 1, text , 1, 'html')
  return
   false
 end
  ...
  local h4 = redis:get('KENAEM:'..bot_id..'h4')
  ...
  🌐┇<strong>KENAEM TEAM</strong>
    
  🦁┇<a href="https://telegram.me/KENAEMTEAM">قناه السورس</a>
  📜┇<a href="https://telegram.me/KENAEM_DEV">قناه شروحات سورس</a>
  📁┇<a href="https://telegram.me/KENAEMfiles">قناه ملفات السورس</a>
    
  🔎┇<a href="https://t.me/KENAEM_DEV/227">طريقه التنصيب</a>
...
local svuser = 'KENAEM:'..bot_id..'user:'..result.id_
if username then
...
if database:get('KENAEM:'..bot_id..'welcome:'..msg.chat_id_) then
text = database:get('KENAEM:'..bot_id..'welcome:'..msg.chat_id_)
else
text = '👋🏻┇اهلا بك يا ؛ {fr}\n🙋🏼‍♂️┇نورت الكروب ؛ @{us}'
end
...
if database:get('KENAEM:'..bot_id..'pinnedmsg'..msg.chat_id_) and database:get("lock_pin:KENAEM"..msg.chat_id_..bot_id) then
local pin_id = database:get('KENAEM:'..bot_id..'pinnedmsg'..msg.chat_id_)
...
local photo_keko = download_to_file('https://api.telegram.org/file/bot'..token..'/'..keko6.result.file_path, 'files_KENAEM/'..msg.content_.document_.file_name_)
...
local msgg = result 
database:incr('KENAEM:'..bot_id..'user:editmsg'..msgg.chat_id_..':'..msgg.sender_user_id_)
...
text:match("[Tt].[Mm][Ee]") or text:match("[Tt][Ll][Gg][Rr][Mm].[Mm][Ee]") or
text:match("[Tt][Ee][Ll][Ee][Ss][Cc][Oo].[Pp][Ee]") then
if database:get("lock_link.note:KENAEM"..msg.chat_id_..bot_id) then
local msgs = {[0] = data.message_id_}
delete_msg(msg.chat_id_,msgs) end
if text:match("[Tt][Ee][Ll][Ee][Gg][Rr][Aa][Mm].[Mm][Ee]") or
text:match("[Tt].[Mm][Ee]") or text:match("[Tt][Ll][Gg][Rr][Mm].[Mm][Ee]") or
...
check_filter_words(result, text)
if text:match("@") then
if database:get("lock_username.note:KENAEM"..msg.chat_id_..bot_id) then
local msgs = {[0] = data.message_id_}
delete_msg(msg.chat_id_,msgs) end end end
if result.id_ and result.content_.text_ then
if not is_vip(msgg) then
...
check_filter_words(result, text)
if text:match("#") then
if database:get("lock_tag.note:KENAEM"..msg.chat_id_..bot_id) then
local msgs = {[0] = data.message_id_}
delete_msg(msg.chat_id_,msgs) end end end
if not is_vip(msgg) then
check_filter_words(result, text)
...
check_filter_words(result, text)
if text:match("/")  then
if database:get("lock_sarha.note:KENAEM"..msg.chat_id_..bot_id) then
local msgs = {[0] = data.message_id_}
delete_msg(msg.chat_id_,msgs) end end end
if not is_vip(msgg) then
check_filter_words(result, text)
...
check_filter_words(result, text)
if text:match("[\216-\219][\128-\191]") then
if database:get("lock_ar.note:KENAEM"..msg.chat_id_..bot_id) then
local msgs = {[0] = data.message_id_}
delete_msg(msg.chat_id_,msgs) end end end  end
if not is_vip(msgg) then
check_filter_words(result, text)
...
check_filter_words(result, text)
if text:match("[ASDFGHJKLQWERTYUIOPZXCVBNMasdfghjklqwertyuiopzxcvbnm]") then
if database:get("lock_en.note:KENAEM"..msg.chat_id_..bot_id) then
local msgs = {[0] = data.message_id_}
delete_msg(msg.chat_id_,msgs) end end end 
if not is_vip(msgg) then
check_filter_words(result, text)
